...
import '../provider/model/amity_message_model.dart' as amm;
import '../provider/model/amity_response_model.dart';
...
class AmityChatRepoImp implements AmityChatRepo {
  ...
  Future<void> fetchChannelById(
      ...
      required Function(amm.AmityMessage?, String?) callback}) async {
    ...
    socket.emitWithAck('v3/message.query', {
      ...
      if (amityResponse.status == "success") {
        //success
        dynamic amityMessages = amm.AmityMessage.fromJson(responsedata!.json!);
  ...
  @override
  Future<void> listenToChannel(Function(amm.AmityMessage) callback) async {
    log("listenToChannelById...");
    socket.on('message.didCreate', (data) async {
      dynamic messageObj = await amm.AmityMessage.fromJson(data);
  ...
  Future<void> sendImageMessage(String channelId, String text,
      Function(amm.AmityMessage?, String?) callback) async {
