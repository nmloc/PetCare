...
class AmityChatRepoImp implements AmityChatRepo {
  ...
  Future<void> fetchUsersByChannelId({
    String? paginationToken,
    int? limit = 30,
    required String channelId,
    required Function(List<AmityUser> userList) setDataFn,
  }) async {
    print("Fetch users in channel id...");
    List<AmityUser> users = [];
    socket.emitWithAck('v3/channel.queryUsers', {
      "channelId": "$channelId",
      "options": {"last": limit, "token": paginationToken}
    }, ack: (data) async {
      print("Fetch users success...");
      var channelUsers = data['data']['channelUsers'] as List;
      UserRepository repository = AmityCoreClient.newUserRepository();
      for (int i = 0; i < channelUsers.length; i++) {
        var user = ChannelUsers.fromJson(channelUsers[i]);
        AmityUser amityUser = await repository.getUser(user.userId!);
        users.add(amityUser);
      }
  ...
  Future<void> fetchChannelById(
      ...
      required Function(
        ...
        String?,
          ...
          callback}) async {
    print("fetchChannelById..." + channelId);
